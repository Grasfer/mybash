...
if [[ ! -d "$LINUXTOOLBOXDIR/mybash" ]]; then
    ...
    git clone https://github.com/Grasfer/mybash "$LINUXTOOLBOXDIR/mybash"
    if [[ $? -eq 0 ]]; then
...
installDepend() {
    ...
    DEPENDENCIES='bash bash-completion tar tree multitail fastfetch tldr trash-cli bat jq software-properties-common'
